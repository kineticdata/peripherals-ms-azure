...
class AzureVmRetrieveV1
  ...
  def execute
    ...
    resource = RestClient::Resource.new("#{url}/#{subscription_id}/services/hostedservices/#{cloud_service_name}/deployments/#{vm_deployment}",
    ...
    json = Crack::XML.parse(response)["Deployment"]["RoleInstanceList"]["RoleInstance"]
    ...
    <<-RESULTS
    <results>
      <result name="vm_info">#{machine.to_json}</result>
    </results>
    RESULTS
